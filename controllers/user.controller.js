...
exports.UpdateUserProfile = async (req, res) => {
  ...
  const response2 = await fetch(
    `http://blockchainexpert.co.in:7000/api/eth/getBalance/${req.body.bnb_address}`,
    {
      method: 'GET',
      headers: {
...
exports.updatePassword = async (req, res) => {
  ...
  let oldPassword = CryptoJS.SHA256(req.body.old_password).toString(
    CryptoJS.enc.Hex
  );
...
exports.getMntWalletsDetails = async (req, res) => {
  ...
  let getMntWalletDetails = await UserModel.getMntWalletDetails(
    req.body.user_id
  );
...
exports.getDirectReferralCount = async (req, res) => {
  let getDirectReferralCount = await UserModel.getDirectReferralCount(
    req.user_id
  );
...
exports.updatekyc = async (req, res) => {
  try {
    ...
    let kyc_document_image = !req.files['kyc_document_image']
      ? null
      : req.files['kyc_document_image'][0].filename;
    let kyc_document_image2 = !req.files['kyc_document_image2']
      ? null
      : req.files['kyc_document_image2'][0].filename;
